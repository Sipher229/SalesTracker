...
function Footer() {
  return (
    <footer className="w-full h-auto flex flex-col bg-gradient-to-br from-mygreen-750 to-mygreen-500 lg:px-8 sm:px-4 lg:pt-10 sm:pt-4 pb-5">
        <div className="w-full h-18 flex items-start justify-between">
            <h1 name="logo" className="roboto-bold sm:text-3xl lg:text-4xl text-center text-myyellow">SalesVerse</h1>
            <div className="w-auto h-auto text-white flex justify-center gap-3">
                ...
                <Link to={"/contact"} className="underline active:no-underline">Feedback</Link>
            </div>
        </div>
        <p className="w-full text-white text-center">Copyright © {new Date().getFullYear()}</p>
    </footer>
  )
}
